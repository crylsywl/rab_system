...
// ===== GET PARAMETERS =====
if (!isset($_GET['id_rab']) || !isset($_GET['id_version'])) {
    die("Parameter tidak lengkap");
}
$id_rab = mysqli_real_escape_string($conn, $_GET['id_rab']);
$id_version = mysqli_real_escape_string($conn, $_GET['id_version']);

// ===== LOAD EXISTING DATA =====
$rab_query = "SELECT * FROM rab WHERE id_rab = ? AND id_version = ?";
$stmt_rab = $conn->prepare($rab_query);
$stmt_rab->bind_param("ss", $id_rab, $id_version);
$stmt_rab->execute();
$rab_data = $stmt_rab->get_result()->fetch_assoc();

if (!$rab_data) {
    die("Data RAB tidak ditemukan");
}

// ===== LOAD EXISTING ITEMS =====
$items_query = "SELECT * FROM isirab WHERE id_rab = ? AND id_version = ? ORDER BY category, material_name";
$stmt_items = $conn->prepare($items_query);
$stmt_items->bind_param("ss", $id_rab, $id_version);
$stmt_items->execute();
$items_data = $stmt_items->get_result()->fetch_all(MYSQLI_ASSOC);

// ===== GROUP ITEMS BY CATEGORY AND BAGIAN =====
$categories = [];
foreach ($items_data as $item) {
    $category_key = $item['bagian'] . '_' . $item['category'];
    ...
    $categories[$category_key]['items'][] = $item;
}

// ===== EDIT RAB FUNCTION =====
if (isset($_POST["edit"])) {
    $id_version_new = generateVersionId();
    ...
    $jumlahTotalBudget = str_replace(".", "", $_POST["grandTotalBudget"] ?? "0");
    $pembulatanBudget = str_replace(".", "", $_POST["pembulatanBudget"] ?? "0");
    $permeterBudget = str_replace(".", "", $_POST["permeterBudget"] ?? "0");
    
    $jumlahTotalAdditional = str_replace(".", "", $_POST["grandTotalAdditional"] ?? "0");
    $pembulatanAdditional = str_replace(".", "", $_POST["pembulatanAdditional"] ?? "0");
    $permeterAdditional = str_replace(".", "", $_POST["permeterAdditional"] ?? "0");

    try {
        $conn->begin_transaction();

        // Step 1: Set current version to not latest
        $stmt_update = $conn->prepare("UPDATE rab SET is_latest = FALSE WHERE id_rab = ? AND is_latest = TRUE");
        $stmt_update->bind_param("s", $id_rab);
        $stmt_update->execute();
        ...
        $original_created_at = $rab_data['created_at'];
        $formatted_date = date('d M Y H:i:s', strtotime($original_created_at));
        $version_description = "Edited from version {$formatted_date}";
        $stmt_rab->bind_param(
            "ssssiisiiiiiss", 
            $id_rab, $id_version_new, $id_user, $projectName, 
            $unit, $type, $location,
            $jumlahTotalBudget, $pembulatanBudget, $permeterBudget,
            $jumlahTotalAdditional, $pembulatanAdditional, $permeterAdditional,
            $version_description
        );
        $stmt_rab->execute();
        ...
        if (!empty($_POST["material_name"])) {
            ...
            $categories_form = $_POST["category"] ?? [];
            ...
            $categoryMap = [];
            foreach ($categories_form as $idx => $categoryName) {
                $categoryId = $_POST["category_id"][$idx] ?? $idx;
                $categoryMap[$categoryId] = mysqli_real_escape_string($conn, $categoryName);
            }
            ...
            foreach ($materials as $idx => $materialName) {
                if (empty(trim($materialName))) continue;
                ...
                $total = (int) str_replace(".", "", $_POST["total_cost"][$idx] ?? "0");
                ...
                // Get bagian
                $bagian = mysqli_real_escape_string($conn, $material_bagians[$idx] ?? "Budget");
                ...
                $stmt_item->bind_param(
                    "sssssssiii",
                    $id_rab, $id_version_new, $id_material, $bagian, $category,
                    $materialName, $unitVal, $qty, $price, $total
                );
                $stmt_item->execute();
            }
        }
        ...
        $conn->commit();
        echo "<script>alert('RAB berhasil di-update! Versi baru telah dibuat.'); window.location='listbackuprab.php?id=$id_rab';</script>";
        exit();

    } catch (Exception $e) {
        $conn->rollback();
        die("Gagal mengupdate RAB: " . $e->getMessage());
    }
}

// Konfigurasi untuk modal material
...
<head>
  ...
  <title>Edit RAB - <?= htmlspecialchars($rab_data['project_name']) ?></title>
...
<main id="main-content" 
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-semibold text-[#FB8E1B]">Edit RAB - <?= htmlspecialchars($rab_data['project_name']) ?></h1>
      <p class="text-sm text-gray-600">Edit RAB untuk project. Perubahan akan membuat versi baru.</p>
    </div>
  ...
  <div>
    ...
    <form action="" method="POST">
        <div class="bg-white rounded-[10px] p-6 shadow-md min-w-[450px] max-w-screen w-full">
            ...
            <!-- TAB 1: Info -->
            <div id="contentInfo" class="flex flex-col gap-4">
                <div>
                    <h1 class="text-xl font-semibold text-[#FB8E1B]">Information</h1>
                    <p class="text-sm text-gray-600">Edit info untuk RAB ini.</p>
                </div>
                <div>
                  ...
                  <input type="text" name="project_name" required
                         ...
                         value="<?= htmlspecialchars($rab_data['project_name']) ?>"
                         required>
                </div>
                ...
                <div>
                  ...
                  <input type="number" name="unit" id="unit" required min="0"
                         class="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-[#023936] outline-none"
                         value="<?= htmlspecialchars($rab_data['unit']) ?>"
                         required>
                </div>
                ...
                <div>
                  ...
                  <input type="number" name="type" id="type" required min="0"
                         class="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-[#023936] outline-none"
                         value="<?= htmlspecialchars($rab_data['type']) ?>"
                         required>
                </div>
                ...
                <div>
                  ...
                  <input type="text" name="location" required
                         class="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-[#023936] outline-none"
                         value="<?= htmlspecialchars($rab_data['location']) ?>"
                         required>
                </div>
            </div>
            ...
            <div id="contentBudget" class="hidden">
                <div id="budget-container" class="w-full">
                    <?php
                    // Load existing Budget categories and items
                    foreach ($categories as $category_key => $category_data):
                        if ($category_data['bagian'] === 'Budget'):
                    ?>
                    <div class="category-block flex flex-col mb-4 bg-[#e7e7e7] p-[24px] shadow-inner rounded-[10px] w-full" data-category-id="<?= $category_key ?>">
                        <div class="flex items-center justify-between w-full gap-4 mb-2">
                            <input type="text" name="category[]" 
                                ...
                                value="<?= htmlspecialchars($category_data['category']) ?>"
                                required>
                            <button type="button" 
                                class="flex bg-[#B3261E] text-white font-medium px-4 py-2 rounded-md hover:bg-[#C65C56] transition removeCategory">
                                <img src="../assets/image/delete_white.svg" alt="delete" class="mr-[8px]"> Hapus Kategori
                            </button>
                        </div>

                        <div class="category-items" data-category-id="<?= $category_key ?>">
                            <?php foreach ($category_data['items'] as $item): ?>
                            <div class="row grid grid-cols-12 gap-2 items-center w-full mt-2" data-category-id="<?= $category_key ?>">
                                <!-- Material -->
                                <div class="flex items-center col-span-5 gap-2">
                                    <input name="material_name[]" 
                                        ...
                                        value="<?= htmlspecialchars($item['material_name']) ?>"
                                        required>
                                    <button type="button" 
                                ...
                                <input type="hidden" name="id_material[]" value="<?= htmlspecialchars($item['id_material']) ?>">
                                <input type="hidden" name="material_category_id[]" value="<?= $category_key ?>">
                                <input type="hidden" name="material_bagian[]" value="Budget">
                            </div>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <input type="hidden" name="bagian[]" value="Budget">
                        <input type="hidden" name="category_id[]" value="<?= $category_key ?>">
                    </div>
                    <?php
                        endif;
                    endforeach;
                ...
                <img src="../assets/image/add.svg" alt="add" class="mr-[8px]"> Tambah Kategori
                </button>
                ...
                <div class="flex flex-col gap-3 w-fit">
                    ...
                    <div class="flex items-center justify-between w-full gap-4">
                        ...
                        value="<?= number_format($rab_data['jumlah_total_budget'], 0, ',', '.') ?>"
                        >
                        <input type="hidden" name="grandTotalBudget" id="hiddenGrandBudget" value="<?= $rab_data['jumlah_total_budget'] ?>">
                    </div>
                    ...
                    <div class="flex items-center justify-between w-full gap-4">
                        ...
                        class="w-56 border border-gray-300 rounded-md p-2 text-right focus:ring-2 focus:ring-[#023936] outline-none"
                        value="<?= number_format($rab_data['pembulatan_budget'], 0, ',', '.') ?>"
                        required>
                    </div>
                    ...
                    <div class="flex items-center justify-between w-full gap-4">
                        ...
                        class="w-56 border border-gray-300 rounded-md p-2 text-right focus:ring-2 focus:ring-[#023936] outline-none"
                        value="<?= number_format($rab_data['permeterpersegi_budget'], 0, ',', '.') ?>"
                        >
                    </div>
                </div>
            </div>
            ...
            <div id="contentAdditional" class="hidden">
              <div id="additional-container" class="w-full">
                    <?php
                    // Load existing Additional categories and items
                    foreach ($categories as $category_key => $category_data):
                        if ($category_data['bagian'] === 'Additional'):
                    ?>
                    <div class="category-block flex flex-col mb-4 bg-[#e7e7e7] p-[24px] shadow-inner rounded-[10px] w-full" data-category-id="<?= $category_key ?>">
                        <div class="flex items-center justify-between w-full gap-4 mb-2">
                            <input type="text" name="category[]" 
                        ...
                        <div class="category-items" data-category-id="<?= $category_key ?>">
                            ...
                            <div class="row grid grid-cols-12 gap-2 items-center w-full mt-2" data-category-id="<?= $category_key ?>">
                                ...
                                <input type="hidden" name="id_material[]" value="<?= htmlspecialchars($item['id_material']) ?>">
                                <input type="hidden" name="material_category_id[]" value="<?= $category_key ?>">
                                <input type="hidden" name="material_bagian[]" value="Additional">
                            </div>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <input type="hidden" name="bagian[]" value="Additional">
                        <input type="hidden" name="category_id[]" value="<?= $category_key ?>">
                    </div>
                    <?php
                        endif;
                ...
                <div class="flex flex-col gap-3 w-fit">
                    ...
                    <div class="flex items-center justify-between w-full gap-4">
                        ...
                        class="w-56 border border-gray-300 rounded-md p-2 text-right focus:ring-2 focus:ring-[#023936] outline-none"
                        value="<?= number_format($rab_data['jumlah_total_additional'], 0, ',', '.') ?>"
                        >
                        <input type="hidden" name="grandTotalAdditional" id="hiddenGrandAdditional" value="<?= $rab_data['jumlah_total_additional'] ?>">
                    </div>
                    ...
                    <div class="flex items-center justify-between w-full gap-4">
                        ...
                        class="w-56 border border-gray-300 rounded-md p-2 text-right focus:ring-2 focus:ring-[#023936] outline-none"
                        value="<?= number_format($rab_data['pembulatan_additional'], 0, ',', '.') ?>"
                        required>
                    </div>
                    ...
                    <div class="flex items-center justify-between w-full gap-4">
                        ...
                        class="w-56 border border-gray-300 rounded-md p-2 text-right focus:ring-2 focus:ring-[#023936] outline-none"
                        value="<?= number_format($rab_data['permeterpersegi_additional'], 0, ',', '.') ?>"
                        >
                    </div>
                </div>
            </div>
        ...
        <div class="flex justify-end gap-3 mt-4">
            <a href="listbackuprab.php?id=<?= urlencode($id_rab) ?>" type="button"
        ...
        <table class="w-full">
          ...
          <tbody id="tableBody" class="divide-y divide-gray-300">
            <?php if (empty($material_data)): ?>
              <tr><td colspan="<?= $role === "admin" ? "8" : "7" ?>" class="py-4 text-center text-gray-500">Data material kosong.</td></tr>
            <?php else: ?>
              <?php foreach ($material_data as $row): ?>
                <tr class="border-b hover:bg-[#E6EBEB]">
...
<!-- SCRIPT SAMA PERSIS DENGAN create_rab.php -->
<!-- Copy semua JavaScript dari create_rab.php di sini -->
<!-- Script Tab Switching, Hitung Otomatis, Tambah Kategori, Modal, dll. -->
...
// Hitung ulang totals saat halaman load
document.addEventListener('DOMContentLoaded', function() {
    // Hitung ulang semua row totals
    document.querySelectorAll('.row').forEach(row => {
        updateRowTotal(row);
    });
    
    // Hitung ulang category subtotals
    document.querySelectorAll('.category-block').forEach(categoryBlock => {
        updateCategorySubtotal(categoryBlock);
    });
